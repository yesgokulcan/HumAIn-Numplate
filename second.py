...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    for possiblePlate in listOfPossiblePlates:
        possiblePlate.imgGrayscale, possiblePlate.imgThresh = before_all.preprocess(possiblePlate.imgPlate)
        if run.showSteps == True:
            cv2.imshow("5a", possiblePlate.imgPlate)
            cv2.imshow("5b", possiblePlate.imgGrayscale)
            cv2.imshow("5c", possiblePlate.imgThresh)
        possiblePlate.imgThresh = cv2.resize(possiblePlate.imgThresh, (0, 0), fx = 1.6, fy = 1.6)
        thresholdValue, possiblePlate.imgThresh = cv2.threshold(possiblePlate.imgThresh, 0.0, 350, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
...
def findPossibleCharsInPlate(imgGrayscale, imgThresh):
    ...
    for contour in contours:
        possibleChar = char_reg.PossibleChar(contour)
